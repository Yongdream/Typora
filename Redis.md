...
- **节点长度管理**：在 ListPack 中，每个节点只记录当前节点的长度，而不需要记录前一个节点的长度。这意味着，当您向列表中添加新元素时，只有新节点的长度字段会受到影响，之前节点的长度保持不变。
- **避免连锁反应**：传统的 **~~压缩列表~~** 在更新时，可能会导致前一个节点的长度字段需要被更新，特别是在插入或删除操作时。这种连锁更新会引起性能下降，因为需要遍历和修改多个节点。listpack 每个节点独立管理自己的长度  
